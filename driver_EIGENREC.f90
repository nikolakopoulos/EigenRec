...
!Define variables!
!----------------!
INTEGER                                           :: N, len, IERR, atoi, status, I, J, RANK, SIZE, dummy, nev, lanczos_steps
INTEGER                                           :: choice, counter, lansteps_init
DOUBLE PRECISION                                  :: atof, tol, t1, t2
...
DOUBLE PRECISION, DIMENSION(201)                  :: xi
DOUBLE PRECISION, DIMENSION(:), ALLOCATABLE       :: mu, yi
...
!------------------------!
tol = 1.0D-6                     ! tolerance for the sought eigenpairs

choice = 1D0
IF (choice == 1D0) THEN
   ALLOCATE(timings_lanczos(7))  ! a vector that keeps track of the time spent in different tasks
   timings_lanczos = 0.0D0
   nev = 50D0                    ! max number of eigenvectors computed
   IF (lanczos_steps .EQ. 0D0) THEN
      lanczos_steps = 50D0       ! Lanczos steps
   ENDIF
   ALLOCATE(W(local_rows, nev))  ! to keep the approximate eigenvectors
   ALLOCATE(Da(lanczos_steps))   ! to kep the Ritz values
   ...
   IF (RANK == 0) THEN
      WRITE(*, *) " "
      WRITE(*, *) "/--------------------------------------------/"
      WRITE(*, *) "Lanczos terminated in: ", t2 - t1, "seconds."
      WRITE(*, *) "Number of converged eigenpairs: ", nev
      WRITE(*, *) "Time spent on the MV products: ", timings_lanczos(1)
